    ...
    foreach ($lists as $list) {
      if ($list->uid == $USER->id || has_capability('mod/sassessment:teacher', $context)) { // Only for Teachers and owners
        ...
        for($i=1;$i<=10;$i++){
          if (!empty($list->{'var'.$i}) || !empty($list->{'file'.$i})){
            ...
            if ($response = $DB->get_record("sassessment_responses", array("aid"=>$sassessment->id, "iid"=>$i, "rid"=>1))) 
              $o .= '<span style="color: #0099CC;">'.round(sassessment_similar_text($response->text, $list->{'var'.$i}))."%</span> &nbsp;";
        ...
        if ($sassessment->textcomparison == 1) {
          ...
          $cell3 = new html_table_cell("<div style=\"color: #0099CC;\">Total: <b>".round($percent)."%</b></div>".$comparecurrent);
          //$cells[] = $cell3;
        }
        
        if ($sassessment->textanalysis == 1) {
          if ($sassessment->textcomparison == 1)
            $cell4 = new html_table_cell("<div style=\"color: #0099CC;\">Total: <b>".round($percent)."%</b></div>"."<small>".sassessment_analizereport(json_decode($list->analize, true))."</small>");
          else
...
if ($a == "add") {
  class sassessment_comment_form extends moodleform {
    function definition() {
      ...
      $o  = "<style> .activeborder{ border: 2px solid #000; }  .activeborderb{ border: 2px solid #2700FF; }</style>";
      ...
      for($i=1;$i<=10;$i++){
        if (!empty($sassessment->{'var'.$i}) || !empty($sassessment->{'file'.$i})) {
          ...
          $o .= '>
            ...
            <td colspan="4">'.$sassessment->{'var'.$i}." ";
          ...
          $o .= '<td style="width: 70px;" id="answer_score_'.$i.'"></td>';
          ...
          /*
            $o .= '<div id="answerbox_'.$i.'">
            
            <div class="fitem femptylabel"><div class="fitemtitle"><div class="fstaticlabel"><label>Your answer</label></div></div><div class="felement fstatic">
